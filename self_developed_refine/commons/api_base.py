...
from self_developed_refine.commons.wait_utils import wait_for_condition
...
class APITestBase:
    ...
    def query_database_with_time(
        ...
        # ------------------------------ 改造1：生成固定时间范围（轮询开始时计算一次） ------------------------------
        # 以“轮询开始时间”为基准，计算 [start_timestamp, end_timestamp]（时间范围：±time_range分钟）
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    result = self._execute_query(
                        ...
                        order_by=order_by,
                        # 关键：传入固定时间范围，不再传time_range（避免动态计算）
    ...
    def _execute_query(
            ...
            order_by: str,
            # ------------------------------ 改造4：参数替换（删除time_range，新增固定时间范围） ------------------------------

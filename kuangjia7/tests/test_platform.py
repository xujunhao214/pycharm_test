...
def test_platform_list(session):
    global platformList
...
def test_create_platform(session):
    data = {
        "brokerName": "测试",
        "platformType": "MT4",
...
def test_update_platform(session):
    data = {
        "id": list_id,
        "brokerName": "测试编辑",
        "platformType": "MT4",
...
@pytest.mark.dependency(depends=["create_platform"])
def test_platfor_list2(session):
    global list_id
    parms = {
        "page": 1,
        "limit": 50,
...
@pytest.mark.dependency(depends=["create_platform"])
def test_delete_platform(session):
    data = [list_id]

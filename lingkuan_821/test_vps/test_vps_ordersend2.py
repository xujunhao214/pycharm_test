...
class TestVPSOrderSend(APITestBase):
    ...
    def test_dbquery_addsalve_orderSendclose(self, var_manager, db_transaction):
        ...
        with allure.step("2. 验证主指令开仓数据"):
            ...
            # 验证总订单数
            if trader_ordersend["totalNum"]:
                total_orders = db_data[0]["total_orders"]
                self.verify_data(
                    actual_value=float(total_orders),
                    expected_value=float(trader_ordersend["totalNum"]),
                    op=CompareOp.EQ,
                    message="总订单数应符合预期",
                    attachment_name="总订单数详情"
                )
                logging.info(f"总订单数验证通过: {trader_ordersend['totalNum']}")

            # 验证总手数
            if trader_ordersend["totalSzie"]:
                total_lots = db_data[0]["total_lots"]
                self.verify_data(
                    actual_value=float(total_lots),
                    expected_value=float(trader_ordersend["totalSzie"]),
                    op=CompareOp.EQ,
                    message="总手数应符合预期",
                    attachment_name="总手数详情"
                )
                logging.info(f"总手数验证通过: {trader_ordersend['totalSzie']}")

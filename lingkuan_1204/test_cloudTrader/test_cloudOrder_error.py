...
class TestCloudOrdersenderror:
    ...
    # @pytest.mark.skipif(True, reason=SKIP_REASON)
    @allure.story("场景5： 云策略列表-跟单账号超过最大手数")
    @allure.description("""
    ### 测试说明
    - 前置条件：有云策略和云跟单
      1. 获取策略账号服务器最大手数
    ...
    # @pytest.mark.skipif(True, reason=SKIP_REASON)
    @pytest.mark.usefixtures("class_random_str")
    class TestCloudOrderSend5(APITestBase):
        ...
        def test_dbquery_maxlots(self, class_random_str, var_manager, db_transaction):
            ...
            with allure.step("5. 数据库的SQL查询"):
                ...
                sql = f""" SELECT * From follow_platform where server = %s """
                params = (addCloud_MT5Slave["platform"],)
            ...
            with allure.step("6. 提取数据并保存"):
                ...
                if MT5max_lots is None:
                    ...
                    logger.info(f"服务器{addCloud_MT5Slave['platform']}无最大手数数据，已保存空值")
                    allure.attach("None", f"服务器{addCloud_MT5Slave['platform']}的最大手数",
                                  allure.attachment_type.TEXT)
                else:
                    var_manager.set_runtime_variable("MT5max_lots", MT5max_lots)
                    logger.info(f"服务器{addCloud_MT5Slave['platform']}的最大手数是：{MT5max_lots}")
                    allure.attach(str(MT5max_lots), f"服务器{addCloud_MT5Slave['platform']}的最大手数",
                                  allure.attachment_type.TEXT)

        # @pytest.mark.skipif(True, reason=SKIP_REASON)
        @allure.title("云策略列表-策略开仓")

...
from lingkuan_721.commons.redis_utils import *
...
class TestLeakageopen(APITestBase):
    ...
    # ---------------------------
    # @pytest.mark.skip(reason=SKIP_REASON)
    @allure.title("redis校验，和数据库的数据做比对")
    def test_dbquery_redis(self, var_manager, db_transaction, redis_order_data_send):
        ...
        with allure.step("5. 比较Redis与数据库数据"):
            ...
            db_comparable_list = [
                {
                    "order_no": record["ticket"],
                    "magical": record["magic"],
                    "size": float(record["lots"]),  # 确保类型一致
                    "open_price": float(record["open_price"]),  # 确保类型一致

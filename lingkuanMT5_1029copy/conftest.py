...
from lingkuanMT5_1029copy.commons.mfa_key import generate_code
from lingkuanMT5_1029copy.commons.Encryption_and_decryption import aes_encrypt_str
from lingkuanMT5_1029copy.commons.session import EnvironmentSession
from lingkuanMT5_1029copy.commons.variable_manager import VariableManager
from lingkuanMT5_1029copy.commons.test_tracker import TestResultTracker
from lingkuanMT5_1029copy.commons.feishu_notification import send_feishu_notification
from lingkuanMT5_1029copy.commons.enums import Environment
from lingkuanMT5_1029copy.config import ENV_CONFIG  # 仅导入配置数据
from lingkuanMT5_1029copy.commons.redis_utils import RedisClient, get_redis_client
from typing import List, Dict, Any
from pathlib import Path
import sys
from DBUtils.PooledDB import PooledDB
from lingkuanMT5_1029copy.commons.random_generator import generate_random_str
...
def api_session(environment) -> EnvironmentSession:
    ...
    session = EnvironmentSession(
        ...
        MT5vps_url=config.get("MT5vps_url")
    )
    yield session
    session.close()
...
def logged_session(api_session, var_manager, request, environment):
    ...
    if url_marker and url_marker.args[0] == "vps":
        api_session.use_MT5vps_url()
        logger.info(f"切换到VPS URL: {api_session.MT5vps_url}")
...
def redis_order_data_send(redis_client, var_manager) -> List[Dict[str, Any]]:
    ...
    redis_key = var_manager.get_variable("redis_order_key",
                                         default=f"follow:repair:send:{IP_ADDRESS}#{NODE}#{NODE}#{MT5vps_user_accounts_1}#{new_user['account']}")
...
def redis_order_data_close(redis_client, var_manager) -> List[Dict[str, Any]]:
    ...
    # 从变量管理器中获取Redis键（也可硬编码或通过参数传入）
    redis_key = var_manager.get_variable("redis_order_key",
                                         default=f"follow:repair:close:{IP_ADDRESS}#{NODE}#{NODE}#{MT5vps_user_accounts_1}#{new_user['account']}")
    hash_data = redis_client.get_hash_data(redis_key)
...
@pytest.fixture(scope="function")
def redis_MT5cloudTrader_data_send(redis_client, var_manager) -> List[Dict[str, Any]]:
    """获取Redis中的订单数据（可直接在测试用例中使用）"""
    ...
    # 从变量管理器中获取Redis键（也可硬编码或通过参数传入）
    redis_key = var_manager.get_variable("redis_order_key",
                                         default=f"followcloud:repair:send:{cloudMaster_id}#{MT5cloudTrader_traderList_4}#{MT5cloudTrader_traderList_2}")
    hash_data = redis_client.get_hash_data(redis_key)
...
@pytest.fixture(scope="function")
def redis_MT5cloudTrader_data_close(redis_client, var_manager) -> List[Dict[str, Any]]:
    """获取Redis中的订单数据（可直接在测试用例中使用）"""
    MT5cloudTrader_traderList_2 = var_manager.get_variable("MT5cloudTrader_traderList_2")
    MT5cloudTrader_traderList_4 = var_manager.get_variable("MT5cloudTrader_traderList_4")
    cloudMaster_id = var_manager.get_variable("cloudMaster_id")
    # 从变量管理器中获取Redis键（也可硬编码或通过参数传入）
    redis_key = var_manager.get_variable("redis_order_key",
                                         default=f"followcloud:repair:close:{cloudMaster_id}#{MT5cloudTrader_traderList_4}#{MT5cloudTrader_traderList_2}")
    hash_data = redis_client.get_hash_data(redis_key)

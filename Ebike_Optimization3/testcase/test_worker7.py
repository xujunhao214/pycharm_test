...
@allure.epic("订单主流程-客服后台点击到达")
@allure.title("客服后台-点击已到达")
def test_repairer_arrive():
    timestamp = str(int(time.time() * 1000))
    data = {
    ...
    with allure.step("1. 点击已到达"):
        url = PROJCET_URL + "/support/order/repairer-arrive"
        r1 = ak.post(url=url, json=data, headers=headers)
    with allure.step("2. 结果校验"):
        results = ak.get_text(r1.text, "$.msg")
        assert "OK" == results
...
@allure.epic("订单主流程-客服后台点击到达")
@allure.title("客服后台-再次点击已到达")
def test_repairer_arrive2():
    timestamp = str(int(time.time() * 1000))
    data = {
        "repairOrderId": reorderId,
        "arriveTime": timestamp

...
def test_finish_order():
    ...
    with allure.step("1. 全部订单-订单列表-点击结束订单"):
        url = PROJCET_URL + "/support/order/finish-order"
        r1 = ak.post(url=url, json=data, headers=headers)
    with allure.step("2. 结果校验"):
        results = ak.get_text(r1.text, "$.msg")
        assert "OK" == results

        ...
        def test_query_blocked_password(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not blocked_password_list:
                    ...
                    attach_body = f"封闭状态查询-是，返回 {len(blocked_password_list)} 条记录，blocked_password值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(blocked_password_list)])
        ...
        def test_query_blocked_passwordno(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not blocked_password_list:
                    ...
                    attach_body = f"封闭状态查询-否，返回 {len(blocked_password_list)} 条记录，blocked_password值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(blocked_password_list)])
            ...
            with allure.step("3. 查询校验"):
                ...
                if not blocked_password_extra_list:
                    ...
                    attach_body = f"封闭状态说明-已解封，返回 {len(blocked_password_extra_list)} 条记录，blocked_password_extra值如下：\n" + \
                                  "\n".join(
                                      [f"第 {idx + 1} 条：{s}" for idx, s in enumerate(blocked_password_extra_list)])

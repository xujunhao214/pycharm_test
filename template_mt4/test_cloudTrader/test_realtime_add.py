...
@allure.feature("跟单管理-实时跟单")
class Test_create:
    @allure.story("实时跟单查询校验")
        ...
        def test_query_following_mode(self, var_manager, logged_session, status, status_desc):
            ...
            with allure.step(f"3. 查询结果校验：返回记录的following_mode应为{status}"):
                ...
                if not status_list:
                    ...
                    attach_body = f"跟随模式查询-{status_desc}，返回 {len(status_list)} 条记录，following_mode值如下：\n" + \
        ...
        def test_query_direction(self, var_manager, logged_session, status, status_desc):
            ...
            with allure.step(f"3. 查询结果校验：返回记录的direction应为{status}"):
                ...
                if not status_list:
                    ...
                    attach_body = f"跟随方向查询-{status_desc}，返回 {len(status_list)} 条记录，direction值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(status_list)])
        ...
        def test_query_pause(self, var_manager, logged_session, status, status_desc):
            ...
            with allure.step(f"3. 查询结果校验：返回记录的pause应为{status}"):
                ...
                if not status_list:
                    ...
                    attach_body = f"暂停查询-{status_desc}，返回 {len(status_list)} 条记录，pause值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(status_list)])
        ...
        def test_query_create_time(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not create_time_list:
                    ...
                    attach_body = f"跟随时间查询-开始时间：{ONE_HOUR_AGO}，返回 {len(create_time_list)} 条记录，create_time值如下：\n" + \
        ...
        def test_query_master_account(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not master_account_list:
                    ...
                    attach_body = f"喊单者账户查询：{trader_account}，返回 {len(master_account_list)} 条记录，master_account值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(master_account_list)])
        ...
        def test_query_master_nickname(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not master_nickname_list:
                    ...
                    attach_body = f"喊单者昵称查询：{trader_master_nickname}，返回 {len(master_nickname_list)} 条记录，master_nickname值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(master_nickname_list)])
        ...
        def test_query_nickname(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not nickname_list:
                    ...
                    attach_body = f"跟单昵称查询：{follow_nickname}，返回 {len(nickname_list)} 条记录，nickname值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(nickname_list)])
        ...
        def test_query_master_server(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not master_server_list:
                    ...
                    attach_body = f"喊单者服务器查询：{trader_master_server}，返回 {len(master_server_list)} 条记录，master_server值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(master_server_list)])
        ...
        def test_query_slave_server(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not slave_server_list:
                    ...
                    attach_body = f"跟单者服务器查询：{follow_slave_server}，返回 {len(slave_server_list)} 条记录，slave_server值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(slave_server_list)])
        ...
        def test_query_master_id(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not master_id_list:
                    ...
                    attach_body = f"喊单者ID查询：{trader_pass_id}，返回 {len(master_id_list)} 条记录，master_id值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(master_id_list)])
        ...
        def test_query_slave_id(self, var_manager, logged_session):
            ...
            with allure.step("3. 查询校验"):
                ...
                if not slave_id_list:
                    ...
                    attach_body = f"跟单ID查询：{follow_pass_id}，返回 {len(slave_id_list)} 条记录，slave_id值如下：\n" + \
                                  "\n".join([f"第 {idx + 1} 条：{s}" for idx, s in enumerate(slave_id_list)])

...
from lingkuan_refine.commons.wait_utils import wait_for_condition
    ...
    def wait_for_database_no_record(
            ...
            try:
                ...
                if len(result) == 0:
                    if stable_start_time is None:
                        ...
                        elapsed = time.time() - start_time
                        logger.info(
                            f"[{self._get_current_time()}] 无记录稳定{stable_period}秒，符合预期（耗时{elapsed:.1f}秒）")
                        if attach_to_allure:
                            with allure.step("等待结果：符合预期（无记录）"):
                                allure.attach(f"无记录稳定{stable_period}秒", "结果说明", allure.attachment_type.TEXT)
        ...
        if attach_to_allure:
            with allure.step("超时后最终结果"):
                allure.attach(f"超时{timeout}秒，查到{len(final_result)}条记录", "结果说明", allure.attachment_type.TEXT)

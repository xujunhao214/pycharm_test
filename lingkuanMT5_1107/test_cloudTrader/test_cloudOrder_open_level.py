...
from lingkuanMT5_1107.commons.redis_utils import *
...
class TestMT5cloudTrader_openandlevel:
    ...
    class TestMT5cloudTrader_open(APITestBase):
        ...
        def test_dbMT5cloudTrader_cloudBatchUpdate(self, class_random_str, var_manager, db_transaction):
            with allure.step("1. 查询数据库验证是否修改成功"):
                ...
                params = (MT5cloudTrader_user_accounts_4, cloudMaster_id,)
        ...
        @allure.title("云策略-云策略列表-修改云跟单")
        def test_MT5cloudTrader_cloudBatchUpdate2(self, class_random_str, var_manager, logged_session):
        ...
        @allure.title("数据库校验-云策略列表-修改云跟单账号是否成功")
        def test_dbMT5cloudTrader_cloudBatchUpdate2(self, class_random_str, var_manager, db_transaction):
            with allure.step("1. 查询数据库验证是否修改成功"):
                MT5cloudTrader_user_accounts_4 = var_manager.get_variable("MT5cloudTrader_user_accounts_4")
                cloudMaster_id = var_manager.get_variable("cloudMaster_id")
                sql = f"SELECT * FROM follow_cloud_trader WHERE account = %s and cloud_id = %s"
    ...
    class TestMT5cloudTrader_level(APITestBase):
        ...
        def test_dbquery_addsalve_clsesdetail(self, class_random_str, var_manager, db_transaction):
            ...
            with allure.step("3. 提取数据"):
                MT5cloudTrader_master_order_level = [record["master_order"] for record in db_data]
                var_manager.set_runtime_variable("MT5cloudTrader_master_order_level", MT5cloudTrader_master_order_level)
        ...
        def test_follow_repairSend(self, class_random_str, var_manager, logged_session):
            with allure.step("1. 发送平仓补全请求"):
                ...
                MT5cloudTrader_traderList_2 = var_manager.get_variable("MT5cloudTrader_traderList_2")
                MT5cloudTrader_traderList_4 = var_manager.get_variable("MT5cloudTrader_traderList_4")
                MT5cloudTrader_master_order_level = var_manager.get_variable("MT5cloudTrader_master_order_level")
                for MT5cloudTrader_master_order_level in MT5cloudTrader_master_order_level:
                    data = [
                        {
                            "cloudId": cloudMaster_id,
                            "repairType": 1,
                            "masterId": MT5cloudTrader_traderList_2,
                            "slaveId": MT5cloudTrader_traderList_4,
                            "masterOrder": MT5cloudTrader_master_order_level
                        }
                    ]
        ...
        def test_MT5cloudTrader_cloudOrderClose2(self, class_random_str, logged_session, var_manager):
            MT5cloudTrader_user_ids_2 = var_manager.get_variable("MT5cloudTrader_user_ids_2")
            # 1. 发送平仓请求
            data = {
                "isCloseAll": 1,

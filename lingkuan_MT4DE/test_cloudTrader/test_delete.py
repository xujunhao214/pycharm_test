...
class TestDelete_cloudTrader(APITestBase):
    ...
    def test_delete_cloudBatch(self, logged_session, var_manager):
        ...
        for i in range(5, cloudTrader_user_count + 1):
            with allure.step(f"删除第{i}云跟单账号"):
                ...
                if not slave_id:
                    print(f"未找到需要删除的账号ID：cloudTrader_traderList_{i}")
    ...
    # @pytest.mark.skip(reason=SKIP_REASON)
    @allure.title("数据库校验-云策略列表-删除MT5云跟单账号")
    def test_dbdelete_MT5cloudBatch(self, var_manager, db_transaction):
        ...
        try:
            ...
            allure.attach(f"云跟单账号 {cloudTrader_MT5traderID} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
        except TimeoutError as e:
            allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
            print(f"删除失败: {str(e)}")
    ...
    # @pytest.mark.skip(reason=SKIP_REASON)
    @allure.title("数据库校验-VPS数据-删除MT5跟单账号")
    def test_dbdelete_addMT5Slave(self, var_manager, db_transaction):
        ...
        try:
            ...
            allure.attach(f"云跟单账号{addCloud_MT5Slave['account']} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
        except TimeoutError as e:
            allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
            print(f"删除失败: {str(e)}")
    ...
    def test_dbdelete_MT5user(self, var_manager, db_transaction):
        ...
        try:
            ...
            allure.attach(f"账号 {addCloud_MT5Slave['account']} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
        except TimeoutError as e:
            allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
            print(f"删除失败: {str(e)}")
    ...
    def test_dbdelete_belowVps(self, var_manager, db_transaction):
        ...
        if cloudTrader_user_count < 10:  # 修改条件为至少10个账号
            print(f"用户总数需至少为10，当前为{cloudTrader_user_count}，无法提取后9个数据进行校验")
        ...
        for i in range(2, cloudTrader_user_count + 1):  # 循环索引2-10（共9次）
            with allure.step(f"验证第{i}个账号是否下架成功"):
                ...
                if not account:
                    print(f"未找到需要验证的账号：cloudTrader_user_accounts_{i}")
    ...
    def test_delete_userlist(self, logged_session, var_manager):
        ...
        for i in range(1, cloudTrader_user_count + 1):
            with allure.step(f"删除第{i}个账号"):
                ...
                if not user_id:
                    print(f"未找到第{i}个账号的ID（变量名：cloudTrader_user_ids_{i}）")
    ...
    def test_dbdelete_userlist(self, var_manager, db_transaction):
        ...
        for i in range(1, cloudTrader_user_count + 1):
            with allure.step(f"验证第{i}个账号的删除状态"):
                ...
                account = var_manager.get_variable(f"cloudTrader_user_accounts_{i}")
                if not account:
                    print(f"未找到第{i}个账号的账号名（变量名：cloudTrader_user_accounts_{i}）")

...
class TestDeleteUser(APITestBase):
    ...
    def test_dbdelete_addsalve(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否删除成功"):
            ...
            try:
                ...
                allure.attach(f"跟单账号 {vps_user_accounts_1} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
            except TimeoutError as e:
                allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
                print(f"删除失败: {str(e)}")
    ...
    def test_dbdelete_addMT5Slave(self, var_manager, db_transaction):
        ...
        sql = f"SELECT * FROM follow_trader WHERE account = %s"
        params = (addVPS_MT5Slave["account"],)
        try:
            self.wait_for_database_deletion(
                db_transaction=db_transaction,
                sql=sql,
            ...
            allure.attach(f"云跟单账号{addVPS_MT5Slave['account']} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
        except TimeoutError as e:
            allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
            print(f"删除失败: {str(e)}")
    ...
    def test_dbdelete_MT5user(self, var_manager, db_transaction):
        ...
        try:
            ...
            allure.attach(f"账号 {addVPS_MT5Slave['account']} 已成功从数据库删除", "验证结果", allure.attachment_type.TEXT)
        except TimeoutError as e:
            allure.attach(f"删除超时: {str(e)}", "验证结果", allure.attachment_type.TEXT)
            print(f"删除失败: {str(e)}")
    ...
    def test_delete_addsalvelist(self, var_manager, logged_session):
        ...
        for i in range(1, vps_addslave_count + 1):
            with allure.step(f"删除第{i}个跟单账号"):
                ...
                if not slave_id:
                    print(f"未找到需要删除的账号ID：vps_addslave_ids_{i}")
    ...
    def test_dbdelete_addsalvelist(self, var_manager, db_transaction):
        ...
        for i in range(1, vps_addslave_count + 1):
            with allure.step(f"验证第{i}个账号是否删除成功"):
                ...
                if not account:
                    print(f"未找到需要验证的账号：vps_user_accounts_{i}")
    ...
    def test_delete_userlist(self, logged_session, var_manager):
        ...
        for i in range(1, vps_user_count + 1):
            with allure.step(f"删除第{i}个账号"):
                ...
                if not user_id:
                    print(f"未找到第{i}个账号的ID（变量名：vps_user_ids_{i}）")
    ...
    def test_dbdelete_userlist(self, var_manager, db_transaction):
        ...
        for i in range(1, vps_user_count + 1):
            with allure.step(f"验证第{i}个账号的删除状态"):
                ...
                if not account:
                    print(f"未找到第{i}个账号的账号名（变量名：vps_user_accounts_{i}）")

...
#
#         # 记录请求行
#         logger.info(f"{request_time} 发送请求 >>>> 方法={method} URL={url}")
#
#         # 记录请求头
#         headers = kwargs.get("headers", {})
#         logger.info(f"{request_time} 发送请求 >>>> 请求头={headers}")
#
#         # 记录请求体（处理不同类型的请求体）
...
#             # 限制大请求体的日志长度
#             body_str = str(body)
#             logger.info(f"{request_time} 发送请求 >>>> 请求体={body_str[:1000]}{'...' if len(body_str) > 1000 else ''}")
#
...
#         # 记录响应状态码和耗时
#         logger.info(
#             f"{response_time} 接收响应 <<<< 状态码={response.status_code} 耗时={elapsed_time.total_seconds() * 1000:.2f}ms")
#
#         # 记录响应头
#         logger.info(f"{response_time} 接收响应 <<<< 响应头={dict(response.headers)}")
#
#         # 记录响应体（尝试以文本形式记录，失败则以二进制长度记录）
#         try:
#             content = response.text
#             logger.info(f"{response_time} 接收响应 <<<< 响应体={content[:1000]}{'...' if len(content) > 1000 else ''}")
#         except UnicodeDecodeError:
#             logger.info(f"{response_time} 接收响应 <<<< 响应体=[二进制数据，长度={len(response.content)}]")
import os
import datetime
import logging
import requests
...
class JunhaoSession(requests.Session):
    ...
    def _log_request(self, method, url, kwargs, start_time):
        ...
        if body:
            ...
            # 限制大请求体的日志长度
            logger.info(
                f"[{self.log_module}] {request_time} 发送请求 >>>> 请求体={body_str[:1000]}{'...' if len(body_str) > 1000 else ''}")
    ...
    def _log_response(self, response, elapsed_time):
        ...
        try:
            ...
            logger.info(
                f"[{self.log_module}] {response_time} 接收响应 <<<< 响应体={content_str[:1000]}{'...' if len(content_str) > 1000 else ''}")
        except UnicodeDecodeError:
            logger.info(

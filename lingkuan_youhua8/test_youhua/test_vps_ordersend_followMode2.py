# lingkuan_youhua8/tests/test_vps_loukai.py
import allure
import logging
import pytest
from lingkuan_youhua8.VAR.VAR import *
from lingkuan_youhua8.conftest import var_manager
from lingkuan_youhua8.commons.api_base import APITestBase  # 导入基础类
...
class TestLoukai(APITestBase):
    ...
    def test_dbquery_updateslave(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否修改成功"):
            ...
            logging.info(f"模式改为净值比例2: {followMode}")
            var_manager.set_runtime_variable("followMode", followMode)

            if db_data[0]["follow_mode"] == 2:
                allure.attach("数据修改成功", "成功详情", allure.attachment_type.TEXT)
            else:
                error_msg = "数据修改失败"
                allure.attach(error_msg, "修改失败", allure.attachment_type.TEXT)

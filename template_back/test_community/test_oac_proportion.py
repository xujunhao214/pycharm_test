import time
from template_back.commons.api_base import APITestBase, CompareOp, logger
import allure
import logging
import logging
import datetime
...
from template_back.commons.session import percentage_to_decimal
from template_back.public_function.proportion_public import PublicUtils


@allure.feature("跟随方式-按比例")
class Test_proportionall:
    ...
    class Test_orderseng1(APITestBase):
        ...
        @pytest.mark.url("vps")
        @allure.title("自研跟单-跟单软件看板-VPS数据-策略开仓")
        ...
        @allure.title("自研跟单-数据库校验-策略开仓-提取订单号和手数")
        ...
        def test_query_opentrader_getRecordList(self, var_manager, logged_session):
            ...
            with allure.step(f"3. 数据校验"):
                with allure.step("喊单者手数校验-开仓手数和持仓订单手数"):
        ...
        def test_query_openfollow_getRecordList(self, var_manager, logged_session):
            ...
            with allure.step(f"3. 数据校验"):
                with allure.step("跟单手数校验-开仓手数和持仓订单手数"):
                    totalLots = self.json_utils.extract(response.json(), "$.records[0].totalLots")
                    logging.info(f"手数是: {totalLots}")
                    ...
                    logger.info(f"跟单者手数：{totalLots} 开仓手数：{expected_lots_open}")

                # @pytest.mark.skipif(True, reason="跳过此用例")

        @pytest.mark.url("vps")
        @allure.title("自研跟单-跟单软件看板-VPS数据-策略平仓")
    ...
    @allure.story("场景2：跟随方式-按比例-固定比例50%")
    @allure.description("""
    ### 测试说明
    - 前置条件：有喊单账号、跟单账号，跟单已经和喊单有订阅关系
      1. 修改订阅信息，跟随方式-按比例-固定比例50%
      2. 自研跟单-VPS策略账号进行开仓，总手数0.2
      3. 账号管理-持仓订单-喊单和跟单数据校验
      4. 跟单管理-开仓日志-喊单和跟单数据校验
      5. 跟单管理-VPS管理-喊单和跟单数据校验
      6. 自研跟单-VPS策略账号进行平仓
    ...
    class Test_orderseng2(APITestBase):
        ...
        def test_trader_openorderSend(self, var_manager, logged_vps):
            ...
            data = {
                ...
                "startSize": trader_ordersend["startSize2"],
                "endSize": trader_ordersend["endSize2"],
                "traderId": vps_trader_id
            }
            response = self.send_post_request(
                logged_vps,
    ...
    @allure.story("场景3：跟随方式-按比例-固定比例25%")
    @allure.description("""
    ### 测试说明
    - 前置条件：有喊单账号、跟单账号，跟单已经和喊单有订阅关系
      1. 修改订阅信息，跟随方式-按比例-固定比例25%
      2. 自研跟单-VPS策略账号进行开仓，总手数0.01
      3. 账号管理-持仓订单-喊单和跟单数据校验
      4. 跟单管理-开仓日志-喊单和跟单数据校验
      5. 跟单管理-VPS管理-喊单和跟单数据校验
    ...
    @allure.story("场景4：跟随方式-按比例-固定比例100%-跟单方向反向")
    @allure.description("""
    ### 测试说明
    - 前置条件：有喊单账号、跟单账号，跟单已经和喊单有订阅关系
      1. 修改订阅信息，跟随方式-按比例-固定比例100%-跟单方向反向
      2. 自研跟单-VPS策略账号进行开仓，总手数0.01
      3. 账号管理-持仓订单-喊单和跟单数据校验
      4. 跟单管理-开仓日志-喊单和跟单数据校验
      5. 跟单管理-VPS管理-喊单和跟单数据校验

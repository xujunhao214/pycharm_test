...
from template_back.conftest import *
...
class EnvironmentSession_vps(requests.Session):
    """
    支持多环境切换的会话管理类（增强版）
    使用 ConnectionError 捕获包括 DNS 解析失败在内的所有连接异常
    """

    def __init__(self, environment: Environment, vpsbase_url: str, docuvps_url=None):
        self.environment = environment
        self.vpsbase_url = vpsbase_url.rstrip('/')  # 移除vpsbase_url末尾的斜杠
        self.jsonpath_utils = JsonPathUtils()
        self.g_vars: Dict[str, Any] = {}  # 存储全局变量
        self.last_response = None  # 存储最后一次响应
        self.named_urls = {}  # 存储命名URL
        self.docuvps_url = docuvps_url or ENV_CONFIG[environment].get("docuvps_url")
        self.current_url = vpsbase_url  # 默认使用vpsbase_url
        super().__init__()

        # 设置环境标识到日志适配器
        self.logger = logging.LoggerAdapter(logger, {"env": environment.value})
        self.logger.info(f"[{DATETIME_NOW}] 初始化环境会话: {environment.value} | Base URL: {self.vpsbase_url}")

    def use_vpsbase_url(self):
        """切换回默认vpsbase_url"""
        self.current_url = self.vpsbase_url
        self.logger.info(f"[{DATETIME_NOW}] 切换到Base URL: {self.current_url}")
        return self

    def use_docuvps_url(self):
        """切换到docuvps_url"""
        self.current_url = self.docuvps_url

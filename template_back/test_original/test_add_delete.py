...
import requests
import os
from template_back.VAR.VAR import *  # 若URL_TOP等变量在VAR.py，需保留
...
def get_follow_accounts_from_runtime_json():
    ...
    # 若当前脚本路径与VAR目录层级不同，可直接写绝对路径（如"D:/pycharm_test/template_back/VAR/runtime_vars_cloud.json"）
...
class Test_delete_batch(APITestBase):
    ...
    def test_run_public(self, var_manager, logged_session):
        """登录并获取token（若runtime_vars_cloud.json中已有token_top，可省略此步）"""
        public_front = PublicUtils()
        public_front.test_login(var_manager)  # 登录后将token_top存入var_manager

    @allure.title("多账号批量解绑：取消订阅 + 前端解绑 + 数据库校验")
    @pytest.mark.parametrize("account_info", FOLLOW_ACCOUNT_LIST)  # 循环所有账号

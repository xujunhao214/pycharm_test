# lingkuan_711/tests/test_delete.py
import time
...
class TestDeleteTrader(APITestBase):
    ...
    def test_dbdelete_addsalve(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否删除成功"):
            ...
            logging.info(f"查询条件: table=follow_trader, account={add_Slave['account']}")

            sql = f"SELECT * FROM follow_trader WHERE account = %s"
            params = (add_Slave["account"],)
            try:
                ...
                allure.attach(f"跟单账号 {add_Slave['account']} 已成功从数据库删除", "验证结果")
    ...
    def test_dbdelete_vpstrader(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否删除成功"):
            ...
            logging.info(f"查询条件: table=follow_trader, account={vps_trader['account']}")

            sql = f"SELECT * FROM follow_trader WHERE account = %s"
            params = (vps_trader["account"],)
            try:
                self.wait_for_database_deletion(
                    db_transaction=db_transaction,
                    sql=sql,
                ...
                allure.attach(f"策略账号 {vps_trader['account']} 已成功从数据库删除", "验证结果")
            except TimeoutError as e:
                allure.attach(f"删除超时: {str(e)}", "验证结果")
                pytest.fail(f"删除失败: {str(e)}")

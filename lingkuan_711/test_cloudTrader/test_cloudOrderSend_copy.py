# lingkuan_711/tests/test_masOrderSend.py
import allure
import logging
import pytest
import time
...
class TestMasordersend(APITestBase):
    ...
    def test_dbcloudTrader_cloudOrderClose(self, var_manager, db_transaction):
        ...
        with allure.step("3. 校验数据"):
            ...
            assert float(total_cloudOrder) == float(
                totalSzie_cloudOrder), f"跟单总手数和下单的手数不相等 (实际: {total_cloudOrder}, 预期: {totalSzie_cloudOrder})"

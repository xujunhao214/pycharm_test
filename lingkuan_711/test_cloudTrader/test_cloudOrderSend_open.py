# lingkuan_711/tests/test_云策略_ordersend.py
import allure
import logging
import pytest
import time
import math
from lingkuan_711.VAR.VAR import *
from lingkuan_711.conftest import var_manager
from lingkuan_711.commons.api_base import APITestBase  # 导入基础类
...
class TestcloudTrader_open(APITestBase):
    # ---------------------------
    # 云策略-云策略列表-修改云策略跟单traderList_cloudTrader_4
    # ---------------------------
    ...
    def test_bargain_masOrderSend(self, api_session, var_manager, logged_session):
        ...
        data = {
            "traderList": [user_ids_cloudTrader_2],
            "type": 0,
            "tradeType": 1,
            "intervalTime": 100,
    ...
    def test_dbquery_order_detail(self, var_manager, db_transaction):
        ...
        with allure.step("2. 校验数据"):
            ...
            assert math.isclose(
                float(total),
                float(totalSzie),
                rel_tol=1e-9,  # 相对误差容忍度（1e-9表示允许0.0000001%的误差）
                abs_tol=1e-9  # 绝对误差容忍度（适用于接近0的数值）
            ), f"跟单总手数和下单的手数不相等 (实际: {total}, 预期: {totalSzie})"
    ...
    def test_dbquery_open(self, var_manager, db_transaction):

...
def generate_simple_report(allure_results_dir, env, report_path):
    ...
    for root, dirs, files in os.walk(allure_abs_dir):
        for file in files:
            if file.endswith(".json") and "result" in file:
                ...
                # ========== 新增：提取用例的完整方法路径（包含参数化标识） ==========
                # 从case_full_name中提取完整的方法路径（用于匹配耗时记录的case_full_name）
                case_method_full = re.sub(r'^.*?(test_vps\..+)$', r'\1', case_full_name)
                # 记录用例的执行时间（毫秒），用于最终匹配验证
                case_exec_time = case_start

                all_case_results.append({
                    "case_unique_id": case_unique_id,
                    "case_name": case_name,
                    "case_full_name": case_full_name,
                    "case_method_full": case_method_full,  # 新增：完整方法路径
                    ...
                    "stop_time": case_stop,
                    "case_exec_time": case_exec_time  # 用例执行时间戳
                })
    ...
    # 5.2 读取并处理耗时记录（核心修复：按case_full_name的方法名+时间戳匹配）
    case_time_map = {}
    if os.path.exists(time_record_file):
        try:
            ...
            for record in time_records:
                ...
                # 提取耗时记录的唯一标识：方法名 + 最后一段时间戳
                record_unique_key = re.sub(r'^.*#(\d+)$', r'\1', record_full_name)  # 提取时间戳后缀
                record_key = f"{record_method}_{record_unique_key}"  # 如 test_query_logType_1764309490531

                # 存储精准映射
                time_record_mapping[record_key] = elapsed_ms
                # 存储备用映射
                pure_id_to_records[record_pure_id].append({
                    "elapsed_ms": elapsed_ms,
                    "full_name": record_full_name,
                    "unique_key": record_unique_key,
                    "timestamp": record_timestamp,
                    "record_key": record_key
                })

            # ========== 核心修复2：为每个用例精准匹配耗时 ==========
            # 先按执行顺序给同纯标识的用例编号，然后按顺序匹配耗时记录
            pure_id_case_count = defaultdict(int)  # 统计每个纯标识的用例数量
            pure_id_case_list = defaultdict(list)  # 存储每个纯标识的用例列表

            # 第一步：按执行时间排序，分组
            for case in cases:
                pure_id = case["pure_identity"]
                pure_id_case_list[pure_id].append(case)

            # 第二步：对每个纯标识的用例按执行时间排序
            for pure_id in pure_id_case_list:
                pure_id_case_list[pure_id].sort(key=lambda x: x["case_exec_time"])

            # 第三步：按顺序匹配耗时记录
            for pure_id, case_list in pure_id_case_list.items():
                # 获取该纯标识的耗时记录（按时间戳后缀排序）
                record_list = pure_id_to_records.get(pure_id, [])
                # 按时间戳后缀排序耗时记录
                record_list.sort(key=lambda x: x["unique_key"])

                # 按顺序匹配
                for idx, case in enumerate(case_list):
                    if idx < len(record_list):
                        # 匹配成功
                        elapsed_ms = record_list[idx]["elapsed_ms"]
                        case_time_map[case["case_unique_id"]] = elapsed_ms
                        print(f"✅ 耗时匹配成功：{pure_id} | {case['case_name']} → {elapsed_ms}ms")
                    else:
                        # 无匹配记录
                        print(f"⚠️ 无匹配耗时记录：{pure_id} | {case['case_name']}")

            # ========== 打印未匹配的耗时记录 ==========
            # 统计已匹配的纯标识
            matched_pure_ids = set([case["pure_identity"] for case in cases])
            for pure_id, records in pure_id_to_records.items():
                if pure_id not in matched_pure_ids:
                    for record in records:
                        print(f"⚠️ 耗时记录无匹配用例：{pure_id} | {record['full_name']}")
    ...
    # 5.4 耗时统计
    elapsed_list = [case_time_map.get(c["case_unique_id"], 0.0) for c in interface_cases if
                    case_time_map.get(c["case_unique_id"], 0.0) > 0]
    ...
    for case in interface_cases:
        elapsed_ms = case_time_map.get(case["case_unique_id"], 0.0)
        time_details.append({
            "module": case["module"],

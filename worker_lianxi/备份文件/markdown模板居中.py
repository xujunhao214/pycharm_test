                    ...
                    if final_status == "FAILED":
                        ...
                        # 场景3：其他失败（如数据库校验）
                            ...
                            if failure_match:
                                ...
                                # 适配"Failed: 校验失败: xxx"格式
                                failed_match = re.search(r'Failed: 校验失败: ([^|]+)', msg)
                                if failed_match:
                                    failure_msg = failed_match.group(1).strip()[:80]
                                else:
...
if __name__ == "__main__":
    generate_simple_report(
        allure_results_dir="report/cloud_results",  # 替换为实际的allure结果目录
        env="test",  # 环境参数
        report_path="report/Cloud接口自动化测试报告.md"  # 报告输出路径
    )

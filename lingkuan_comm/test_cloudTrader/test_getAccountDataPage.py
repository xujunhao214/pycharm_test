...
class TestCloudOrderSend_newScenarios:
    ...
    - 预期结果：数据正确
    """)
    class TestCloudOrderSend1(APIVPSBase):
        # @pytest.mark.skipif(True, reason="跳过")
        @allure.title("云策略交易下单-分配平仓-防止数据残留")
        def test_copy_orderprevent_close(self, var_manager, logged_vps):
        ...
        def test_dashboard_getAccountDataPage(self, var_manager, logged_vps):
    ...
    - 预期结果：数据正确
    """)
    class TestCloudOrderSend2(APIVPSBase):
        @pytest.mark.skipif(True, reason="跳过")
        @allure.title("云策略交易下单-分配下单请求")
        def test_copy_order_send(self, logged_vps, var_manager):
            # 发送云策略交易下单-复制下单请求

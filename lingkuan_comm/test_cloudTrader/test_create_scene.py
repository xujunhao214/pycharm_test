...
class TestCreate_importcloudTrader(APIVPSBase):
    # @pytest.mark.skip(reason=SKIP_REASON)
    @allure.title("云策略-云策略列表-批量新增云跟单账号")
    def test_create_importcloudBatchAdd(self, var_manager, logged_vps):
        # 1. 获取账号总数和所有ID
    ...
    def test_dbquery_cloudBatchAdd(self, var_manager, dbvps_transaction):
        # 1. 获取账号总数和所有账号信息
    ...
    def test_update_addSlave(self, var_manager, logged_vps):
        ...
        for param in parametrize_data:
            with (allure.step(f"1. 对数据进行参数化修改")):
                ...
                # 发送请求并验证
                response = self.send_post_request(
                    logged_vps, '/mascontrol/cloudTrader/cloudBatchUpdate', json_data=data
                )

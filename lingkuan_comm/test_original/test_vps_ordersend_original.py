...
class VPSOrderTestBase(APIVPSBase):
    ...
    def _send_open_order(self, var_manager, logged_vps):
        """发送开仓请求（公共方法）"""
    ...
    def _verify_open_order_master(self, var_manager, dbvps_transaction):
        """验证主指令开仓数据（公共方法）"""
    ...
    def _verify_open_order_slave(self, var_manager, dbvps_transaction):
        """验证跟单指令开仓数据（公共方法）"""
    ...
    def _verify_close_order_master(self, var_manager, dbvps_transaction):
        """验证主指令平仓数据（公共方法）"""
        new_user = var_manager.get_variable("new_user")
    ...
    def _verify_close_order_slave(self, var_manager, dbvps_transaction):
        """验证跟单指令平仓数据（公共方法）"""
        vps_user_accounts_1 = var_manager.get_variable("vps_user_accounts_1")
...
class TestVPSOrderSend1(VPSOrderTestBase):
    ...
    def test_dbquery_orderSendclose(self, var_manager, dbvps_transaction):
        self._verify_close_order_master(var_manager, dbvps_transaction)

    @allure.title("数据库校验-策略平仓-跟单指令及订单详情数据检查")
    def test_dbquery_addsalve_orderSendclose(self, var_manager, dbvps_transaction):
        self._verify_close_order_slave(var_manager, dbvps_transaction)
...
class TestVPSOrderSend5(VPSOrderTestBase):
    ...
    def test_trader_stopOrder(self, var_manager, logged_vps):
        # 1. 发送策略开仓停止请求
        vps_trader_id = var_manager.get_variable("vps_trader_id")
        params = {
            "type": "0",

...
每种模式均包含下单、数据校验和平仓流程
""")
class TestVPStradingOrders(APIVPSBase):
    """整合VPS分配下单和复制下单的测试类"""

    # -------------------------- 分配下单测试 --------------------------
    @allure.story("模式一：分配下单")
    @allure.title("VPS分配下单请求")
    def test_allocation_order_send(self, logged_vps, var_manager):
        # 发送VPS分配下单请求
    ...
    def test_allocation_open_verify(self, var_manager, dbvps_transaction):
    ...
    def test_allocation_order_close(self, var_manager, logged_vps):
        vps_user_ids_1 = var_manager.get_variable("vps_user_ids_1")
        # 发送平仓请求
        data = {
            "isCloseAll": 1,
    ...
    def test_allocation_close_verify(self, var_manager, dbvps_transaction):
    ...
    def test_copy_open_verify(self, var_manager, dbvps_transaction):
        with allure.step("获取开仓订单数据"):
            vps_user_accounts_1 = var_manager.get_variable("vps_user_accounts_1")
            sql = """
                SELECT 
    ...
    def test_copy_close_verify(self, var_manager, dbvps_transaction):
        with allure.step("获取平仓订单数据"):
            vps_user_accounts_1 = var_manager.get_variable("vps_user_accounts_1")
            vps_addslave_id = var_manager.get_variable("vps_addslave_id")
            sql = """

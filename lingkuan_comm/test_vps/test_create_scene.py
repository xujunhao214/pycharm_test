...
class TestCreate_Scene(APIVPSBase):
    # @pytest.mark.skip(reason=SKIP_REASON)
    @pytest.mark.url("vps")
    @allure.title("新增跟单账号（仅使用后6个数据与模板匹配）")
    def test_import_addSlave(self, var_manager, logged_vps, encrypted_password):
        ...
        for param in parametrize_data:
            with allure.step(f"执行参数: {param['desc']}"):
                ...
                response = self.send_post_request(
                    logged_vps, '/subcontrol/follow/addSlave', json_data=data
                )
    ...
    def test_update_addSlave(self, var_manager, logged_vps, encrypted_password):
        # 1. 获取总用户数（需确保至少有7个，才能取后6个）
        vps_user_count = var_manager.get_variable("vps_user_count", 0)
        if vps_user_count < 7:
            pytest.fail(f"用户总数需至少为7，当前为{vps_user_count}，无法提取后6个数据")
        ...
        for param in parametrize_data:
            with allure.step(f"执行参数: {param['desc']}"):
                ...
                # 发送请求并验证
                response = self.send_post_request(
                    logged_vps, '/subcontrol/follow/updateSlave', json_data=data
                )

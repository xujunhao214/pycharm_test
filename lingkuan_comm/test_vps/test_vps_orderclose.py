...
class TestVPSCoreFunctionality:
    ...
    - 预期结果：平仓的停止功能正确
    """)
    class TestStopCloseFunctionality(APIVPSBase):
        @pytest.mark.url("vps")
        ...
        def test_trader_orderclose2(self, var_manager, logged_vps):
            with allure.step("1. 发送全平订单平仓请求"):
                vps_trader_id = var_manager.get_variable("vps_trader_id")
                new_user = var_manager.get_variable("new_user")
                data = {
...
class TestVPSFollowDirection:
    ...
    - 预期结果：平仓的订单方向功能正确
    """)
    class TestReverseFollowClose_sell(APIVPSBase):
        @pytest.mark.url("vps")
        ...
        def test_trader_symbol(self, var_manager, logged_vps):
        ...
        def test_trader_orderclose1(self, var_manager, logged_vps):
        ...
        def test_trader_orderclose3(self, var_manager, logged_vps):
    ...
    - 预期结果：平仓的订单方向功能正确
    """)
    class TestReverseFollowClose_buy(APIVPSBase):
        @pytest.mark.url("vps")
    ...
    - 预期结果：平仓的订单方向功能正确
    """)
    class TestReverseFollowClose_all(APIVPSBase):
        @pytest.mark.url("vps")
        @allure.title("策略账号开仓操作")
        def test_trader_orderSend(self, var_manager, logged_vps):
            # 1. 发送策略开仓请求
...
class TestVPSOrderQuantityControl:
    ...
    - 预期结果：平仓的订单数量功能正确
    """)
    class TestBatchCloseFunctionality(APIVPSBase):
        @pytest.mark.url("vps")
...
class TestVPSOrderType:
    ...
    """)
    # @pytest.mark.skipif(True, reason=SKIP_REASON)
    class TestMT4ExternalOrderClose(APIVPSBase):
    ...
    class TestVPSInternalOrderType:
        ...
        - 预期结果：平仓的订单类型功能正确
        """)
        class TestInternalOrderClose(APIVPSBase):
            @pytest.mark.url("vps")
            @allure.title("策略开仓")
            def test_trader_orderSend(self, var_manager, logged_vps):
                # 1. 发送策略开仓请求
    ...
    class TestVPSAllOrderType:
        ...
        - 预期结果：平仓的订单类型功能正确
        """)
        class TestAllOrderClose(APIVPSBase):
            @pytest.mark.url("vps")
            @allure.title("策略开仓")
            def test_trader_orderSend(self, var_manager, logged_vps):
                # 1. 发送策略开仓请求
...
class TestVPSCloseRemark:
    ...
    - 预期结果：平仓的备注功能正确
    """)
    class TestFixedCommentMatching(APIVPSBase):
        @pytest.mark.url("vps")

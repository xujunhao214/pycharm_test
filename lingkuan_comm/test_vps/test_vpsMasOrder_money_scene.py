...
class TestVPSMasOrder_money_scene:
    ...
    - 预期结果：三个账号的币种手数正确
    """)
    class TestVPSOrderSend_money(APIVPSBase):
    ...
    - 预期结果：三个账号的手数正确
    """)
    class TestVPSOrderSend_Scence(APIVPSBase):
        # @pytest.mark.skip(reason=SKIP_REASON)
        @pytest.mark.url("vps")
        @allure.title("跟单软件看板-VPS数据-策略开仓")
        def test_trader_orderSend(self, var_manager, logged_vps):
        ...
        def test_vps_dbtrader_euqit(self, var_manager, dbvps_transaction):
        ...
        def test_dbvps_addsalve_euqit(self, var_manager, dbvps_transaction):
        ...
        def test_vps_dbtrader_euqit2(self, var_manager, dbvps_transaction):
            with allure.step("1. 获取订单详情表账号数据"):

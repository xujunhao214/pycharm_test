...
class TestLeakageopen_level:
    ...
    @allure.description("""
        ...
        """)
    class TestLeakageopen(APIVPSBase):
        @pytest.mark.url("vps")
        @allure.title("跟单软件看板-VPS数据-修改跟单账号（漏开）")
        def test_update_slave(self, var_manager, logged_vps, encrypted_password):
        ...
        def test_dbquery_updateslave(self, var_manager, dbvps_transaction):
        ...
        def test_dbquery_redis(self, var_manager, dbvps_transaction, redis_order_data_send):
            with allure.step("1. 获取订单详情表账号数据"):
                trader_ordersend = var_manager.get_variable("trader_ordersend")
                new_user = var_manager.get_variable("new_user")
                symbol = trader_ordersend["symbol"]
        ...
        def test_update_slave2(self, var_manager, logged_vps, encrypted_password):
            # 1. 发送修改vps跟单账号请求,修改followOpen：0关闭  1开启
            add_Slave = var_manager.get_variable("add_Slave")
            vps_user_accounts_1 = var_manager.get_variable("vps_user_accounts_1")
            vps_trader_id = var_manager.get_variable("vps_trader_id")
        ...
        @allure.title("数据库校验-VPS数据-修改跟单账号是否成功")
        def test_dbquery_updateslave2(self, var_manager, dbvps_transaction):
            with allure.step("1. 查询数据库验证是否修改成功"):
                vps_user_accounts_1 = var_manager.get_variable("vps_user_accounts_1")
                sql = f"SELECT * FROM follow_trader_subscribe WHERE slave_account = %s"
                params = (vps_user_accounts_1,)
        ...
        def test_follow_repairSend2(self, var_manager, logged_vps):
            with allure.step("1. 发送开仓补全请求"):
                vps_addslave_id = var_manager.get_variable("vps_addslave_id")
                vps_trader_id = var_manager.get_variable("vps_trader_id")
                data = {
        ...
        def test_trader_orderclose(self, var_manager, logged_vps, dbvps_transaction):
            # 1. 发送全平订单平仓请求
            vps_trader_id = var_manager.get_variable("vps_trader_id")
            new_user = var_manager.get_variable("new_user")
            data = {
    ...
    @allure.description("""
        ...
        """)
    class TestLeakagelevel(APIVPSBase):
        @pytest.mark.url("vps")
        ...
        def test_dbquery_redis(self, var_manager, dbvps_transaction, redis_order_data_close):
            with allure.step("1. 获取订单详情表账号数据"):
                trader_ordersend = var_manager.get_variable("trader_ordersend")
                new_user = var_manager.get_variable("new_user")
                symbol = trader_ordersend["symbol"]
        ...
        @pytest.mark.url("vps")
        def test_follow_orderClose(self, var_manager, logged_vps):
    ...
    """)
    class TestLeakageopen_addstatus(APIVPSBase):
        @pytest.mark.url("vps")
        @allure.title("跟单软件看板-VPS数据-修改策略账号信息")
        def test_subcontrol_trader(self, var_manager, logged_vps, encrypted_password):
        ...
        @pytest.mark.url("vps")
        @allure.title("跟单软件看板-VPS数据-修改策略账号信息")
        def test_subcontrol_trader2(self, var_manager, logged_vps, encrypted_password):
            # 1. 发送修改vps策略的请求，修改followStatus：0关闭 1开启
            with allure.step("发送修改vps策略的请求"):
                new_user = var_manager.get_variable("new_user")
                vps_trader_id = var_manager.get_variable("vps_trader_id")

    ...
    class TestVPSOrderSend1(APITestBase):
        ...
        def test_follow_updateSlave(self, class_random_str, var_manager, logged_session, encrypted_password):
            ...
            with allure.step("3. 修改MT4跟单账号"):
                ...
                data = {
                    ...
                    "id": MT4vps_addslave_id,
                    "platformId": MT4platformId
                }
                response = self.send_post_request(
                    logged_session,
                    '/subcontrol/follow/updateSlave',
        ...
        def test_dbquery_orderSend(self, class_random_str, var_manager, db_transaction):
            ...
            with allure.step("2. 数据校验"):
                trader_ordersend = var_manager.get_variable("trader_ordersend")
                if not db_data:
                    allure.attach("数据库查询结果为空，订单可能没有入库", "失败原因", allure.attachment_type.TEXT)
        ...
        @allure.title("数据库校验-策略开仓-MT4账号跟单指令及订单详情数据检查")
        def test_dbquery_addsalve_MT4orderSend(self, class_random_str, var_manager, db_transaction):
            with allure.step("1. 获取订单详情表账号数据"):
                addVPS_MT4Slave = var_manager.get_variable("addVPS_MT4Slave")
                addVPS_MT4Slave = addVPS_MT4Slave["account"]
                sql = f"""
                            SELECT 
                                fod.size,
                                fod.comment,
        ...
        @allure.title("数据库校验-策略平仓-MT4账号跟单指令及订单详情数据检查")
        def test_dbquery_addsalve_MT4orderSendclose(self, class_random_str, var_manager, db_transaction):
            with allure.step("1. 获取订单详情表账号数据"):
                addVPS_MT4Slave = var_manager.get_variable("addVPS_MT4Slave")
                addVPS_MT4Slave = addVPS_MT4Slave["account"]
                MT4vps_addslave_id = var_manager.get_variable("MT4vps_addslave_id")

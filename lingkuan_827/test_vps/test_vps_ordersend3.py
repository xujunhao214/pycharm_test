        ...
        with allure.step("2. 验证主指令开仓数据"):
            ...
            with allure.step("验证手数范围-开始手数"):
                ...
                self.verify_data(
                    ...
                    expected_value=float(test_params["startSize"]),
                    op=CompareOp.EQ,
                    message="开始手数应符合预期",
            ...
            with allure.step("验证手数范围-结束手数"):
                ...
                self.verify_data(
                    ...
                    expected_value=float(test_params["endSize"]),
                    op=CompareOp.EQ,
                    message="结束手数应符合预期",
            ...
            with allure.step("验证总订单数"):
                total_orders = db_data[0]["total_orders"]
                self.verify_data(
                    actual_value=float(total_orders),
                    expected_value=float(test_params["totalNum"]),
                    op=CompareOp.EQ,
                    message="总订单数应符合预期",
                    attachment_name="总订单数详情"
                )
                logging.info(f"总订单数验证通过: {total_orders}")
            ...
            with allure.step("验证总手数"):
                total_lots = db_data[0]["total_lots"]
                self.verify_data(
                    actual_value=float(total_lots),
                    expected_value=float(test_params["totalSzie"]),
                    op=CompareOp.EQ,
                    message="总手数应符合预期",
                    attachment_name="总手数详情"
                )
                logging.info(f"总手数验证通过: {total_lots}")

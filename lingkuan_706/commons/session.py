...
from lingkuan_706.commons.jsonpath_utils import JsonPathUtils
from lingkuan_706.commons.enums import Environment
from lingkuan_706.conftest import *
...
class EnvironmentSession(requests.Session):
    ...
    def request(self, method: str, url: str, *args, **kwargs) -> requests.Response:
        ...
        # {id(self)} 是 Python 中获取对象唯一标识符
        # request_id = f"REQ-{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}-{id(self)}"
        request_id = f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"

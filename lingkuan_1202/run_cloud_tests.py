...
def run_cloud_tests(env: str = "test"):
    ...
    markdown_report_path = os.path.join(REPORT_ROOT, "Cloud接口自动化测试报告.md")  # Cloud 专属Markdown报告

    # 创建必要目录（确保目录存在，避免报错）
    os.makedirs(report_dir, exist_ok=True)
    log_dir = os.path.join(project_root, "Logs")  # 日志目录改为绝对路径
    ...
    # 生成Cloud独立Allure HTML报告（兼容Linux/Windows）
    try:

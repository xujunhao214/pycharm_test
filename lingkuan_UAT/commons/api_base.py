...
from lingkuan_UAT.commons.wait_utils import wait_for_condition
...
class APITestBase:
    ...
    def assert_data_lists_equal(self, actual: List[Dict], expected: List[Dict],
                                fields_to_compare: List[str], tolerance: float = 1e-9):
        """比较两个数据列表是否相等（支持浮点数容差比较）"""
        # 按ticket排序（确保顺序一致）
        actual_sorted = sorted(actual, key=lambda x: x["ticket"])
        expected_sorted = sorted(expected, key=lambda x: x["ticket"])

        # 检查长度是否一致
        assert len(actual_sorted) == len(expected_sorted), \
            f"数据长度不一致: Redis={len(actual_sorted)}, DB={len(expected_sorted)}"

        # 逐个比较每个订单
        for a, e in zip(actual_sorted, expected_sorted):
            for field in fields_to_compare:
                actual_value = a.get(field)
                expected_value = e.get(field)

                # 处理浮点数比较（使用容差）
                if isinstance(actual_value, float) and isinstance(expected_value, float):
                    assert abs(actual_value - expected_value) <= tolerance, \
                        f"字段 {field} 值不匹配: Redis={actual_value}, DB={expected_value}"
                else:
                    assert actual_value == expected_value, \
                        f"字段 {field} 值不匹配: Redis={actual_value}, DB={expected_value}"

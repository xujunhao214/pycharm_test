    ...
    def test_copy_open_verify(self, var_manager, db_transaction):
        ...
        with allure.step("验证开仓数据"):
            ...
            assert math.isclose(
                ...
                float(db_data[0]["min_lot_size"]),
                rel_tol=1e-9, abs_tol=1e-9
            ), f'结束手数不匹配: 预期0.01, 实际{db_data[0]["min_lot_size"]}'

            assert math.isclose(
                float(0.01),
                float(db_data[0]["max_lot_size"]),
                rel_tol=1e-9, abs_tol=1e-9
            ), f'开始手数不匹配: 预期0.01, 实际{db_data[0]["max_lot_size"]}'
            ...
            assert math.isclose(
                ...
                total_size,
                rel_tol=1e-9, abs_tol=1e-9
            ), f'订单详情总手数不匹配: 预期0.01, 实际{total_size}'
    ...
    def test_copy_close_verify(self, var_manager, db_transaction):
        ...
        with allure.step("验证平仓数据"):
            ...
            assert math.isclose(
                ...
                total_size,
                rel_tol=1e-9, abs_tol=1e-9
            ), f'平仓总手数不匹配: 预期0.01, 实际{total_size}'

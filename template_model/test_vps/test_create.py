# template_model/tests/test_create.py
import time
import pytest
import logging
import allure
...
class TestCreate(APITestBase):
    ...
    def test_create_variety(self, logged_session, var_manager):
        ...
        data = {
            "template_modelName": add_variety["template_modelName"]
        }

        # 1. 添加品种
        response = self.send_post_request(
            logged_session,
            '/mascontrol/variety/addtemplate_model',
            data=data,
            files=files
        )
    ...
    def test_dbquery_variety(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否新增成功"):
            add_variety = var_manager.get_variable("add_variety")
            # 从变量中获取表名和模板名
            template_model_name = add_variety["template_modelName"]
            # 执行数据库查询
            db_data = self.query_database(
                db_transaction,
                f"SELECT * FROM follow_variety WHERE template_model_name = %s",
                (template_model_name,)
            )
            ...
            vps_template_model_id = db_data[0]["template_model_id"]
            logging.info(f"新增品种id: {vps_template_model_id}")
            var_manager.set_runtime_variable("vps_template_model_id", vps_template_model_id)

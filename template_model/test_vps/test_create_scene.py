    ...
    def test_import_addSlave(self, var_manager, logged_session, encrypted_password):
        ...
        template_models: List[Dict[str, Any]] = [
            ...
            },
            {
                "followMode": 1,
                "followParam": "1",
                "template_modelId": vps_template_model_id,
                "Cfd": "",
                "mode_desc": "修改品种（3倍）"
            },
            {
        ...
        for i in range(len(all_accounts)):
            ...
            template_model = template_models[i]  # 直接一一对应（账号1→模板1，账号2→模板2，...）
            parametrize_data.append({
                "account": account,
                "followMode": template_model["followMode"],
                "followParam": template_model["followParam"],
                "template_modelId": template_model["template_modelId"],
                "Cfd": template_model["Cfd"],  # 新增Cfd参数
                "desc": f"账号{account}-{template_model['mode_desc']}"
            })
        ...
        for param in parametrize_data:
            with allure.step(f"执行参数: {param['desc']}"):
                ...
                data = {
                    ...
                    "template_modelId": param["template_modelId"],
                    "followDirection": 0,
                    "remainder": 0,
                    "placedType": 0,
                    "followStatus": 1,
    ...
    def test_update_addSlave(self, var_manager, logged_session, encrypted_password):
        ...
        for i in range(len(all_accounts)):
            ...
            parametrize_data.append({
                ...
                "followParam": template_model["followParam"],
                "template_modelId": template_model["template_modelId"],
                "remark": template_model["remark"],  # 修改备注
                "Cfd": template_model["Cfd"],  # 修改Cfd参数
                "id": id,
                "desc": f"账号{account}-{template_model['mode_desc']}"
            })
        print(f"生成{len(parametrize_data)}条参数化测试数据（后6个账号）")

...
class TestDeleteUser(APITestBase):
    ...
    def test_deletetemplate_model(self, logged_session, var_manager):
        """测试删除用户接口"""
        # 1. 发送删除品种请求
        vps_template_model_id = var_manager.get_variable("vps_template_model_id")
        response = self.send_delete_request(
            logged_session,
            '/mascontrol/variety/deletetemplate_model',
            json_data=[vps_template_model_id]
        )
    ...
    def test_dbdelete_template_model(self, var_manager, db_transaction):
        with allure.step("1. 查询数据库验证是否删除成功"):
            add_variety = var_manager.get_variable("add_variety")
            logging.info(f"查询条件: table=follow_variety, template_modelName2={add_variety['template_modelName']}")

            sql = f"SELECT * FROM follow_variety WHERE template_model_name = %s"
            params = (add_variety["template_modelName"],)
            try:
                self.wait_for_database_deletion(
                    db_transaction=db_transaction,
                    sql=sql,
                ...
                allure.attach(f"品种 {add_variety['template_modelName']} 已成功从数据库删除", "验证结果")
            except TimeoutError as e:
                allure.attach(f"删除超时: {str(e)}", "验证结果")
                pytest.fail(f"删除失败: {str(e)}")

...
from lingkuan_1203.commons.wait_utils import wait_for_condition
import inspect
...
class APITestBase:
    ...
    def TIME_RECORD_FILE(self):
        ...
        - 路径：lingkuan_1129/report/[test_vps/test_cloudTrader/merged_allure-results]/time_record.json
        """
        # 1. 获取当前用例的唯一标识（从调用栈）
        ...
        time_record_file = os.path.abspath(
            os.path.join(
                base_file_dir,
                "../../report",  # 从基类目录向上两级到 lingkuan_1129 根目录
                project_dir,
                "time_record.json"
            )
        )
